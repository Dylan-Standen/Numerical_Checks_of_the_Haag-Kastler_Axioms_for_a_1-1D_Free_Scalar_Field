...
The Haa-Kastler axioms can be written naturally in the language of category theory; hence, it will be useful to define and introduce what each term means. This section does not aim to be an extensive look into the subject; thus, an interested reader should find a more thorough review here \cite{leinster2025basiccategorytheory}.
...
\begin{definition}
    Let $\mathcal{C}$ and $\mathcal{D}$ be categories. Then a (covariant) Functor, $F: \mathcal{C} \rightarrow \mathcal{D}$, is the map such that:
    \begin{itemize}
        \item For all $A \in \ob{\mathcal{C}}$
        \begin{equation*}
            A \mapsto F(A) = B \in \ob{\mathcal{D}}.
        \end{equation*}
    ...
    \begin{itemize}
        ...
        with $\circ_\mathcal{C}, \circ_\mathcal{D}$ representing the composition in $\mathcal{C}$ and $\mathcal{D}$, respectively. The most obvious applied example of this law would be a group homomorphism. Given the groups $(G, \cdot)$ and $(H, *)$ (which we can consider as single object categories with maps corresponding to group elements), the map (functor) $f: G \rightarrow H$ is a group homomorphism iff
        \begin{equation*}
            f(g_1 \cdot g_2) = f(g_1) * f(g_2).
        \end{equation*} 
        \item For all $A \in \ob{C}$,
...
We may similarly define a \textit{contravariant functor} from the category $\mathcal{C}$ to the category  $\mathcal{D}$ as a functor, but with the sources and targets of the morphisms flipped. In other words, it is the functor from $\mathcal{C}^\text{Op} \rightarrow \mathcal{D}$.
Given $f \in \Hom{C}{A}{B}$ with $A,B \in \ob{C}$,

\begin{equation*}
    f \mapsto F(f) \in \Hom{D}{F(B)}{F(A)}\hspace{0.15cm} ,
\end{equation*}
and for $g \in \Hom{C}{B}{C}$ with $B,C \in \ob{C}$

\begin{equation*}
    F(g \circ_{\mathcal{C}}f) = F(f) \circ_\mathcal{D} F(g) \hspace{0.15cm} .
\end{equation*}
...
A clear example of this is the reals with the canonical ``less-than-or-equal-to'' relation $\leq$. The above definition introduces a partial ordering on the set (hence the name); essentially, it allows us to determine how a pair of elements is ordered. This is compared with a totally ordered set. Another conceptually similar and useful object to us is a directed set; 

\begin{definition}
    A \textit{directed set} $D$ is the poset such that 
...
\begin{definition}
    ...
    \begin{equation*}
        \mathcal{A}_\text{alg} : = \left(\bigcup_{U \hspace{0.05cm} \in \hspace{0.05cm} \mathcal{U}(M)}\mathcal{A}(U)\right) / \sim \hspace{0.15cm} ,
    \end{equation*}
...
Poincaré group, such that for all $L\in P$, $O \in\mathcal U(M)$,
and $A\in\mathcal A(O)$,
\begin{equation*}
...
In particular, 
\begin{equation*}
 \pi\big(\mathcal A(LO)\big) = U(L)\hspace{0.15cm}\pi\big(\mathcal A(O)\big)\hspace{0.15cm}U^{-1}(L)
  \hspace{0.15cm} .
\end{equation*}
...
%1 
\bibitem{leinster2025basiccategorytheory}
   Tom Leinster, \emph{``Basic Category Theory''}, 
   (2025) \href{https://arxiv.org/pdf/1612.09375v2}{[\url{https://arxiv.org/pdf/1612.09375v2}]}.
